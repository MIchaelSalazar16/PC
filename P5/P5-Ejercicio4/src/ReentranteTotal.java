...
public class ReentranteTotal implements MonitorArbitraje {
	...
	// Variable para el ejercicio 3
	private HashMap<String,Value> lectoresParaEscribir; 
	
	ReentranteTotal() {
		startTime = System.currentTimeMillis();
		numLectores = new HashMap<String,Value>();
		lectoresParaEscribir = new HashMap<String,Value>();
	}
	...
	public synchronized void entrarLeer() throws InterruptedException {
		...
		if (!numLectores.containsKey(nombreLector)){
			...
			// Si no era un lector, comprobamos si es el escritor actual. Si lo es, puede entrar sin esperar
			// puesto que no hay peligro. Si no es el escritor actual, entonces debera esperar al turno de lectores
			...
			System.out.println("\t\t\t\tReentrada (R) num. "+ (v.numAccesos + 1) + " de "+ nombreLector );
		}
	...
	public synchronized void entrarEscribir() throws InterruptedException {
		...
		if (escritorActual != Thread.currentThread()) {
			...
			// Si el HashMap "numLectores" contiene al thread, es un lector que una
			// vez tiene acceso a lectura, quiere acceso a escritura. Copiamos el valor
			// al HashMap "lectoresParaEscribir" y pasa de Lector a Escritor en espera
			if (numLectores.containsKey(nombre))
				lectoresParaEscribir.put(nombre, numLectores.remove(nombre));
			...
			// Una vez ha conseguido el acceso a escritura, podemos devolverlo al conjunto
			// de lectores
			if (lectoresParaEscribir.containsKey(nombre))
				numLectores.put(nombre, lectoresParaEscribir.remove(nombre));
			...
			System.out.println("\t\t\t\tReentrada (W) num. "+ (accesosEscritura + 1) + " de "+ escritorActual.getName() );
		
		// Contar las reentradas de escritura
		accesosEscritura++;
	...
	public synchronized void salirEscribir() {
		...
		// Actualizar numero de reentradas en escritura
		accesosEscritura--;
