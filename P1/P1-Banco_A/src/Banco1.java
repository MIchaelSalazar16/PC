...
class Cajero {
	...
	public void realizar_reintegro(Cuenta cta, int cantidad, Cliente reintegrador) {
		...
		synchronized(cta){
			...
			while (cta.saldo(this) < cantidad){
				  try{
					  System.out.println("Cajero $" + id + ": No hay saldo suficiente en la cuenta #" + cta +
							  ". El Cliente " + reintegrador + " debe esperar...");
					  sinFondos = true;
					  cta.wait();
				  }
			...
			if (sinFondos)
				System.out.println("Cajero $" + id + ": El Cliente " + reintegrador + " finalizo su espera!"); 
